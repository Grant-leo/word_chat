...
from pipeline_runner.artifacts import write_content_artifacts, write_format_artifacts
...
from pipeline_runner.verification import double_verify
...
def run(
    ...
    if md_file and (OPTIONAL_DEPS.extract_md_format is None or OPTIONAL_DEPS.extract_md_content is None):
        print(f'[ERROR] md_parser.py 不可用，不能处理 Markdown 单文件模式。{OPTIONAL_DEPS.optional_import_detail("md_parser")}')
        return None
    if (use_md_format or use_md_content) and (OPTIONAL_DEPS.extract_md_format is None or OPTIONAL_DEPS.extract_md_content is None):
        print(f'[ERROR] md_parser.py 不可用，不能处理 Markdown 输入。{OPTIONAL_DEPS.optional_import_detail("md_parser")}')
        return None
