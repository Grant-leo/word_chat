...
3. Verify outputs
4. Help fine-tune by updating the core pipeline scripts, then re-running the pipeline
...
- You may run `python Outputs/<latest>/build_generated.py` for debugging, but do not make it the permanent edit point
...
`build_generated.py` is generated by `script_generator.py`. It is an output artifact, not the long-term source of truth.
...
6. **Verify the new output directory** — do not patch generated build files or generated docx files directly
...
2. Add the reusable implementation to the appropriate core pipeline script
3. Adapt parameters to come from `format.json`, `content.json`, or derived rules
4. Re-run the whole pipeline

Example: template has no cross-references → generated script has no `B_ref()`. User says "加参考文献引用". Read 基础操作.md → 交叉引用, move the `add_ref_hyperlink` + `bookmarkStart/End` pattern into `script_generator.py`, then re-run the pipeline.
...
build_generated.py ──→ python run ──→ 最终论文.docx
                           ↓
                   Claude + 基础操作.md
                   (inspect, then update core pipeline)
```
