...
- `comparison_assessment.md` / `comparison_assessment.json`: audit decision layer; records `BLOCKED_UNJUDGEABLE`, `FAILED_AUTOMATIC`, `PASSED_WITH_REVIEW`, `PASSED_MACHINE`, or `APPROVED_DELIVERY`, plus confidence, baseline type, manual-review requirement, review pages, approved deviations, and blocking issue codes. Warning-only runs are review-required, not perfect machine passes.
...
Golden compare-only mode must not create missing baselines; a missing baseline is a warning/review state until a maintainer explicitly reruns with `--update-golden` after manual approval.
...
`inventory.md`, and `review_queue.json`. These reports contain file locations,
structural features, classifications, and issue codes, but not body text. Do not
commit them. Fixed classifications are `template_candidate`,
`content_candidate`, `reference_candidate`, `attachment_or_nonpaper`, and
`unsupported_or_conversion_needed`.

Unsupported explicit inputs such as `.doc`, `.wps`, `.rar`, `.lnk`, `.xlsx`,
bad DOCX, and scanned/protected PDFs must surface issue codes plus concrete next
actions instead of raw tracebacks or silent formatting. High-risk DOCX source
structures such as textboxes, footnotes/endnotes, tracked changes, content
controls, embedded objects, landscape sections, unsupported image formats, and
complex/merged tables must also surface issue codes; recoverable visible text or
notes should be handled by the engine first, with remaining layout/numbering
uncertainty routed to warning review.
For tracked changes, extract deterministic Word final-view content before
handoff: `w:ins` / `w:moveTo` is visible body/table/table-row/table-cell/heading/textbox-recovery
content, while `w:del` / `w:moveFrom` and comment bodies must not leak into the final paper. The source
audit should still surface `TRACKED_CHANGES_PRESENT` / `COMMENTS_PRESENT` so
ordinary users know to accept/reject revisions and confirm comments before
delivery.
