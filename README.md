...
- DOCX 横向表格 section 保护：连续相邻且源页面设置签名一致的横向宽表，即使中间没有桥接说明，也会共用同一个 landscape section；带短说明时也可合并，纸张尺寸或页边距不同则保留独立横向 section，避免第二个表套用错误版心。
...
- DOCX 横向表格富文本媒体/块级源序保护：当同一个 `rich_text.items` 桥接段里同时携带代码、图片、题注、小表等内容时，引擎会按源顺序渲染这些媒体和块级子项，再为后续宽表单独建立横向 section，避免图片/公式收集器或按类型批量输出造成内容错位。
