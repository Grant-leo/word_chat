...
- 缺图、损坏/不支持的本地 Markdown 图片、远程 Markdown 图片 URL、图片抽取失败、公式丢失、表格数量不匹配、占位符残留会进入 QA 报告；本地坏图、GIF/WebP/SVG 等不支持格式、扩展名不匹配或 data URI MIME/真实格式不一致会点名 `CONTENT_IMAGE_UNREADABLE` 并提示重新导出 PNG/JPG，远程图片会点名 `CONTENT_IMAGE_REMOTE_UNSUPPORTED`，不会误导用户等待自动下载。
