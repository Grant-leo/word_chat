...
- DOCX 复杂表格小白指引：当 `COMPLEX_TABLE_UNSUPPORTED` 的 detail 出现 `visible_hmerge_continuations=N` 时，`qa_report` / `qa_repair_plan` 的下一步会明确提示用 Word/WPS 对照原文和最终 DOCX，重点核对这些带可见内容的旧式合并延续单元格没有被吞掉、挪位或重复。
