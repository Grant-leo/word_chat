...
- 中文编码防护：生成脚本 QA 还会阻断 `codecs.__dict__.get("decode")` / `vars(codecs).get("decode")` / 普通容器 `.get(...)` 取出的真实 `codecs.decode`、`operator.attrgetter("decode")(codecs)`、函数默认参数里的 `decoder=codecs.decode` / `module=codecs` / `factory=codecs.getdecoder` / `lookup=codecs.lookup`，以及 `from codecs import *` 后裸调用真实 `decode` / `getdecoder` / `lookup` 等路径，避免 UTF-8 中文 bytes 被 GBK 等错误编码二次解码；星号导入后若同层又覆盖为安全本地 `decode` / `getdecoder` / `lookup`，或 `import codecs` 后同层覆盖为安全自定义模块对象，不会误报；覆盖之前已经发生的真实 `codecs.decode` / `getdecoder` / `lookup` 调用仍会阻断。
