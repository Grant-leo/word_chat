...
- 中文编码防护：生成脚本 QA 还会阻断 `operator.attrgetter("decode")(codecs)`、函数默认参数里的 `decoder=codecs.decode` / `module=codecs` / `factory=codecs.getdecoder` / `lookup=codecs.lookup` 等路径，避免 UTF-8 中文 bytes 被 GBK 等错误编码二次解码；安全自定义对象的同名 `decode` 方法不会被误报。
