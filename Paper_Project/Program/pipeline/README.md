...
- Current full synthetic regression: `428 passed, 0 failed`. Latest additions:
  ...
  UTF-8 Chinese bytes as GBK or another wrong charset; they also follow default
  parameters that hide real `codecs` routes (`decoder=codecs.decode`,
  `module=codecs`, `factory=codecs.getdecoder`, `lookup=codecs.lookup`), and
  auto-landscaped long wide tables no longer default-repeat a first row that
  carries rich table-cell payloads such as images, formulas, notes, rich text,
  or nested tables.
...
- Generated-script star-import codecs guard: structural QA treats
  ...
  before Chinese text-derived bytes can be mis-decoded. Regression:
