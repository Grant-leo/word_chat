...
- Synthetic regression after the Markdown reference-style image fix: `224 passed, 0 failed`.
...
- Markdown reference-style images: `![alt][id]` plus `[id]: path` now copy local images into the content stream; undefined image references become `CONTENT_IMAGE_MISSING` instead of staying as ordinary body text, and reference-definition-like lines inside fenced code blocks stay in code.
...
metadata, local image URI-suffix normalization, reference-style image
definitions, remote-image blocker metadata, UTF-8 BOM-safe YAML/front-format
