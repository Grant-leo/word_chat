...
- Current full synthetic regression: `443 passed, 0 failed`. Latest additions:
  ...
  `routes.setdefault("decode", codecs.decode)` before later subscript calls or
  `.decode(...)` can damage Chinese text. Existing safe `setdefault` entries
  remain unblocked. Landscape-table grouping now recognizes compact `rich_text`
  bridge notes
