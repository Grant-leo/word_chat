...
- Generated-script decoder-factory handoff guard: structural QA now follows decoder objects returned by `codecs.getdecoder(...)` and codec-info objects returned by `codecs.lookup(...)` after they are stored in literal `list` / `tuple` containers or returned by no-required-argument functions, before calls such as `routes[0](payload)` or `get_codec().decode(payload)`. Regression: `qa_flags_generated_script_general_codecs_decoder_factories_text_reencoding`.
