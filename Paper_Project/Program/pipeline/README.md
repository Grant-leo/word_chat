...
- Current full synthetic regression: `431 passed, 0 failed`. Latest additions:
  generated-script decode guards now follow `codecs.__dict__.get("decode")`,
  `vars(codecs).get("decode")`, and ordinary literal-container `.get(...)`
  routes that recover the real `codecs.decode`, plus `from codecs import *` star-import
