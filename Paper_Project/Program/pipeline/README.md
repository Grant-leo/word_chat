...
- DOCX landscape and overwide tables: tables extracted from landscape sections
  ...
  section for the next wide table. Bridge content carrying rich-text image runs
...
- DOCX table-cell images: images inside DOCX body table cells are attached to the source cell's `table_cell_items` instead of being appended after the table; same-paragraph image-before-text and text-before-image ordering is preserved from OOXML run order; images inside supported five-level nested tables stay inside the nested cell, and structural/strict image counts recurse through nested `table_cell_items`.
...
- DOCX nested table-cell inline media/formula/notes: supported five-level nested tables preserve the same source order when a nested cell paragraph mixes text, an inline image, LaTeX, OMML, and a footnote reference.
...
up to five-level nested tables as `table_cell_items` with
...
codes remain warning/review signals for six-level-and-deeper nested /
overwide / irregular
...
omitted leading or trailing grid columns as visible empty cells. Source audit
...
bordered, or five-level nested tables.
