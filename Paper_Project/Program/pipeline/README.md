...
- `artifacts.py`: `format.json`, `content.json`, markdown handoff reports, QA-shaped build-failure/unsafe-script reports, and early format-blocker handoffs.
...
- `build_phase.py`: generated-script creation, pre-execution unsafe Unicode decode blocking, DOCX build execution, and generated-script failure handoff.
- `execution.py`: generated-script subprocess execution, pre-subprocess unsafe Unicode decode blocking, and UTF-8 output decoding.
...
- Current full synthetic regression: `448 passed, 0 failed`. Latest additions:
  visual QA sample-page selection now keeps a later wide-table continuation
  page across multi-section long documents within the 8-page bounded sample set
  while preserving figure/formula primary risk pages. The red/green regression
  is `visual_sample_pages_keep_later_table_continuation_across_sections`.
  content parsing now coalesces converter-style split `vMerge continue` rows
  under a `gridSpan + vMerge restart` cell into a single safe rectangular
  `table_merges` record when all split continuations are empty; visible
  continuation payloads still fail open as normal cells. The red/green
  regression is
  `content_parser_coalesces_split_vmerge_continuations_under_gridspan_restart`.
  ...
  codecs.decode})`, `routes.update(decode=codecs.decode)`, and
  `routes.setdefault("decode", codecs.decode)` before later subscript calls, as
  well as direct-return method routes such as
  `routes.get("decode", codecs.decode)(...)`,
  `routes.setdefault("decode", codecs.decode)(...)`, `routes.pop("decode")(...)`,
  and `modules.get("m", codecs).decode(...)`, plus assigned or higher-order
  handoffs such as `decode_text = routes.get("decode", codecs.decode);
  decode_text(...)` and `apply_decoder(routes.get("decode", codecs.decode),
  ...)`, before they can damage Chinese text. Existing safe
  `setdefault/get/pop` entries remain unblocked.
  Landscape-table grouping now recognizes compact `rich_text`
  bridge notes
  ...
  `callbacks = {"decode": decoder}; callbacks["decode"](value, encoding)`,
  when the call site passes real `codecs.decode`, text-derived UTF-8 Chinese
  bytes, and an unsafe encoding such as `"gbk"`; same-shaped custom safe
  decoders and overwritten safe callback containers remain unblocked. Existing
  generated-script decode guards now also block custom batch wrappers that
  hide payload/encoding pairs inside a paired `rows` parameter, such as
  `for value, encoding in rows: decoder(value, encoding)` and
  `[decoder(value, encoding) for value, encoding in rows]`, when the call site
  passes real `codecs.decode` plus an unsafe paired row such as
  `rows = [(text.encode("utf-8"), "gbk")]`; same-shaped custom safe decoders
  ...
  remain unblocked. Existing generated-script decode guards also block custom
  batch wrappers that
  hide encoding values behind `enumerate(values)` index variables, such as
  `for idx, value in enumerate(values): decoder(value, encodings[idx])`,
  when the call site passes real `codecs.decode` plus an unsafe encoding
  container such as `encodings = ["gbk"]`; same-shaped custom safe decoders
  remain unblocked. Existing generated-script decode guards also block custom
  batch wrappers that
  hide encoding values behind `zip(values, encodings)` loop variables, such as
  `for value, encoding in zip(values, encodings): decoder(value, encoding)`,
  when the call site passes real `codecs.decode` plus an unsafe encoding
  container; same-shaped custom safe decoders remain unblocked. Existing
  generated-script decode guards also block custom batch wrappers with
...
- Generated-script `codecs.__getattribute__` / keyword-unpack decode guard:
  structural QA now treats `codecs.__getattribute__("decode")(payload, ...)`
  and aliases of that call as real `codecs.decode(...)` handoffs. It also
  treats `.decode(**kwargs)`, `operator.methodcaller("decode", **kwargs)`, and
  `text.encode(**kwargs)` followed by another decode as dynamic text re-decode
  boundaries, so hidden encoding parameters cannot silently damage Chinese
  text before the generated DOCX is written. Same-shaped safe custom objects
  remain unblocked. Regression:
  `qa_flags_generated_script_general_codecs_decode_text_reencoding` /
  `qa_flags_generated_script_method_decode_text_reencoding` /
  `qa_does_not_flag_shadowed_method_factory_name_for_safe_decoder`.
- Generated-script default-parameter codecs guard: structural QA also follows
  function defaults that hide the real codecs route. Covered forms include
  `def decode_text(..., decoder=codecs.decode)`, `module=codecs`,
  `factory=codecs.getdecoder`, and `lookup=codecs.lookup` before text-derived
  UTF-8 Chinese bytes are decoded again as GBK or another wrong charset. Same
  shaped safe custom objects remain unblocked. Regression:
  `qa_flags_generated_script_default_parameter_codecs_decode_routes`.
- Generated-script star-import codecs guard: structural QA treats
  `from codecs import *` as importing the real `decode`, `getdecoder`, and
  `lookup` routes, so bare calls such as `decode(payload, "gbk")`,
  `getdecoder("gbk")(payload)`, or `lookup("gbk").decode(payload)` are stopped
  before Chinese text-derived bytes can be mis-decoded. Same-level safe
  shadowing such as `def decode(...)`, `decode = safe_decode`,
  `def getdecoder(...)`, or `def lookup(...)` after the star import is allowed,
...
- Generated-script codecs module-container handoff guard: structural QA now
  ...
  attribute containers such as `def get_modules(): return box.modules`, and
  zero-explicit-argument instance/static/class methods such as
  `holder.get_module()`, `Holder.get_modules()[0]`, and
  `Holder.get_config()["modules"]["module"]`, and bound method aliases taken
  from those real module results, such as `decode_text =
  holder.get_module().decode`, `factory =
  Holder.get_module().getdecoder`, and `lookup =
  Holder.get_config()["modules"]["module"].lookup`, plus
  no-required-argument functions returning those bound methods, such as
  `get_decode() -> holder.get_module().decode`,
  `get_factory() -> Holder.get_module().getdecoder`, and
  `get_lookup() -> Holder.get_config()["modules"]["module"].lookup`;
  ...
  synthetic regression: `448 passed, 0 failed`.
...
- DOCX auto-landscape table header guard: long overwide tables with a short
  text-only first row still get default repeated headers, but a first row that
  carries rich cell payloads such as images, formulas, notes, rich text, or
  nested tables is not guessed as a repeated header unless
  `table_repeat_header_rows` is explicitly set. Regression:
  ...
  operations.
  Converter-style tables whose top-left cell uses `gridSpan + vMerge restart`
  but whose lower continuation row splits the same span into multiple empty
  `vMerge continue` cells are also coalesced into that single rectangular
  `table_merges` record. If any split continuation carries visible text,
  media, formulas, nested tables, or note anchors, parsing fails open and
  preserves the payload as visible cell content.
  ...
  the cell text.
  Empty valid `vMerge continue` cells still follow Word merge semantics, but
  any non-empty continuation payload, including plain text, images, formulas,
  nested tables, or note anchors, fails open as a visible cell so content is
  not silently dropped. Source audit records
  `visible_vmerge_continuation_count`, and QA guidance names
  ...
  Repeated header rows keep `tblHeader`/`cantSplit`; default first-row repeat
  is used only when the first row is short enough to behave like a header.
  Unusually tall first rows and tall non-header rows avoid forced header/repeat
  behavior so Word/WPS can split them across pages instead of forcing large
  blank areas, duplicated body-like headers, or row overflow.
...
- Markdown local image path variants: local paths continue resolving `%20` spaces, `<path with spaces>` wrappers, balanced filename parentheses, optional image titles such as `![鍥綸(path "title")`, and local `?query` / `#fragment` suffixes copied from Markdown tools before checking the filesystem.
...
- Generated-script dynamic `codecs.decode` guard: structural QA now also recognizes statically provable dynamic module/function routes such as `__import__("codecs").decode(...)`, `builtins.__import__("codecs").decode(...)`, aliases assigned from `builtins.__import__` / `importlib.import_module`, direct or assigned `getattr(importlib, "import_module")("codecs")` / `getattr(builtins, "__import__")("codecs")`, `importlib.import_module("codecs").decode(...)`, `codecs.__dict__["decode"](...)`, `codecs.__dict__.get("decode")(...)`, `vars(codecs)["decode"](...)`, `vars(codecs).get("decode")(...)`, `globals()["codecs"].decode(...)` / `locals()["codecs"].decode(...)`, literal dictionaries that store either `codecs`, `codecs.decode`, or their simple aliases and are accessed through `[...]` or `.get(...)`, literal `list` / `tuple` containers holding `codecs.decode` / `getattr(codecs, "decode")`, object attributes and object-alias attributes such as `box.decode = codecs.decode; alias.decode(...)`, `SimpleNamespace(decode=codecs.decode)`, class attributes such as `class Holder: decode = codecs.decode`, simple `__init__` instance attributes such as `self.decode = codecs.decode; holder.decode(...)`, class-alias named and temporary instances such as `Alias = Holder; holder = Alias()` and `Alias().decode(...)`, no-required-argument instance factories such as `def make_holder(): return Holder(); holder = make_holder()`, `@staticmethod` factories such as `holder = Holder.make()`, `@classmethod` temporary factories such as `Holder.make().decode(...)`, direct-child local function return handoffs such as `def build_decoder(): def local(): return codecs.decode; return local()`, no-required-argument functions that return direct-child higher-order wrappers such as `def build_apply(): def apply_decoder(decoder, value, encoding): return decoder(value, encoding); return apply_decoder`, higher-order calls that pass a no-required-argument decode-function result such as `apply_decoder(get_decoder(), payload, "gbk")`, higher-order helpers that receive `codecs`, `get_module()`, or a module-object attribute such as `box.module` before calling `module.decode(payload, encoding)`, `getattr(module, "decode")(payload, encoding)`, or a local alias such as `decode = getattr(module, "decode"); decode(payload, encoding)`, required-argument helpers that return a module-parameter dynamic decode attribute such as `def pick(module): return getattr(module, "decode")` before `pick(codecs)(payload, "gbk")`, required-argument outer helpers whose direct child or nested direct-child closure chain captures `module` and returns `getattr(module, "decode")` before `build(codecs)(payload, "gbk")`, and no-required-argument functions that return `codecs.decode`, `getattr(codecs, "decode")`, `functools.partial(codecs.decode, ...)`, or those attributes before re-decoding UTF-8 Chinese bytes as GBK/Latin-1-style text. Same-shaped local safe `builtins` objects without an actual `import builtins` stay unblocked. Regression: `qa_flags_generated_script_general_codecs_decode_text_reencoding` / `qa_flags_generated_script_partial_codecs_decode_text_reencoding` / `qa_does_not_flag_codecs_module_routes_after_safe_shadowing`.
...
- Generated-script dynamic container-mutation guard: structural QA now recognizes real `codecs.decode` or `codecs` module handoffs after simple `append()`, `extend()`, `insert()`, `dict.update()`, `setdefault()`, and subscript assignment mutations, such as `routes=[]; routes.append(codecs.decode); routes[0](payload, "gbk")`, `routes.extend([codecs.decode])`, `routes.insert(0, codecs.decode)`, `routes={}; routes["decode"]=codecs.decode; routes["decode"](payload, "gbk")`, `routes.update({"decode": codecs.decode})`, `routes.update(decode=codecs.decode)`, `routes.setdefault("decode", codecs.decode)`, and `modules.extend([codecs]); modules[0].decode(payload, "gbk")`. It also recognizes direct-return container methods such as `routes.setdefault("decode", codecs.decode)(payload, "gbk")`, `routes.get("decode", codecs.decode)(payload, "gbk")`, `routes.pop("decode")(payload, "gbk")`, and `modules.get("m", codecs).decode(payload, "gbk")`, plus aliases and higher-order handoffs of those method-return values such as `decode_text = routes.get("decode", codecs.decode); decode_text(payload, "gbk")` and `apply_decoder(routes.get("decode", codecs.decode), payload, "gbk")`. These routes previously had no direct call node and no later subscript access, so a generated script could damage Chinese text before QA noticed. Same-shaped safe custom decoders and existing safe `setdefault/get/pop` values remain unblocked. Regression: `qa_flags_generated_script_general_codecs_decode_text_reencoding` / `qa_does_not_flag_dynamic_container_safe_decoder`.
- Generated-script `operator.attrgetter` decode guard: structural QA also treats
  ...
  again as GBK. Direct chained calls, assigned aliases such as
  `decode_text = attrgetter("decode")(codecs)`, and no-required-argument helper
...
- Generated-script module-container handoff guard: structural QA also follows
  ...
  returns of those attribute containers, plus zero-explicit-argument
  instance/static/class methods returning real `codecs` modules or containers.
  Covered examples include
  `get_modules()[0]` where `get_modules()` returns `[codecs]` or
  `box.modules`, plus direct `box.modules[0]`, `namespace.modules[0]`, and
  `Holder.modules["module"]`, as well as `holder.get_module()`,
  `Holder.get_modules()[0]`, and
  `Holder.get_config()["modules"]["module"]`. This covers `module.decode(...)`,
  `module.getdecoder(...)`, `module.lookup(...).decode(...)` helper bodies, and
  bound method aliases such as `decode_text = holder.get_module().decode`,
  `factory = Holder.get_module().getdecoder`, and
  `lookup = Holder.get_config()["modules"]["module"].lookup`,
  plus no-required-argument functions returning those bound methods before
  immediate use, such as `get_decode()(payload, "gbk")`,
  `get_factory()("gbk")(payload)`, and `get_lookup()("gbk").decode(payload)`,
...
- Content-summary coverage: `鍐呭鎻愬彇.md` renders structured `role="image"` items, including table-cell images, as `[鍥剧墖]` and mentions table-cell image counts in table summaries instead of opaque `[缁撴瀯鍖栧唴瀹筣`.
- Template-instruction cleanup coverage: DOCX template format notes, source-TOC examples, cover field hints, and TOC page-number samples are stripped or ignored before final rendering, so template instructions do not appear in `鏈€缁堣鏂?docx`.
- Semantic heading coverage: structural QA treats common Chinese/English backmatter equivalents such as `Acknowledgements` / `Acknowledgment` / `鑷磋阿`, `References` / `鍙傝€冩枃鐚甡, and `Appendix` / `闄勫綍` as matching headings for `CONTENT_HEADING_MISSING`.
...
- Public-template compatibility suite: 5 public templates 脳 5 synthetic scenarios = `25/25` passed.
- Local DOCX strict QA matrix: 5 DOCX templates 脳 5 DOCX contents = `25/25` passed.
...
Caption detection deliberately separates true captions such as `鍥?1 xxx 绀烘剰鍥綻
from prose references such as `鍥?1 灞曠ず浜?..`, so body prose keeps body style
