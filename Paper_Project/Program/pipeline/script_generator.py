...
def add_heading(text, level):
    ...
    return add_text(normalize_heading_spacing(text), role='h' + str(level), first_indent=(level > 1), outline_level=level - 1)
...
def compute_cover_skip_indices(cover):
    ...
    while j < len(cover) and is_elastic_cover_empty(cover[j]):
        after.append(j); j += 1
    # Do not delete every spacer after the information table.  The template
    # uses those empty paragraphs to separate the committee line from the
    # cover table.  Delete only the excess spacers needed for long titles,
    # while keeping at least one structural blank line before the next text.
    # Keep a small structural gap before the next visible cover paragraph
    # (normally the committee/signature line).  Use only template spacer
    # structure, not school-specific text.
    # Long replacement content must first keep the whole first-page cover
    # structure together.  Adjacent blank paragraphs after the info table are
    # treated as elastic spacers from the template, not semantic content.  The
    # next visible paragraph keeps its own formatting, so no school-specific
    # text or fixed line count is needed here.
    for idx in after:
        skip.add(idx)
...
def add_caption(text, role='figure_caption'):
    p = add_text(normalize_caption(text), role=role, first_indent=False)
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
def formula_latex_from_text(text):
    ...
    return r'\text{' + latex_escape_text(t) + '}'
...
def render_formula(item, chapter=None):
    ...
    if not latex and not xml:
        latex = formula_latex_from_text(text)
    rules = DATA.get('rules') or {}
    if latex and rules.get('formula_numbered') and r'\tag' not in latex and r'\begin{equation}' not in latex and r'\begin{align}' not in latex:
        ch = chapter or 0
        FORMULA_COUNTERS[ch] = FORMULA_COUNTERS.get(ch, 0) + 1
        label = f'{ch}-{FORMULA_COUNTERS[ch]}' if ch else str(FORMULA_COUNTERS[ch])
...
def build_document(toc_page_map=None, native_toc=True):
    ...
    global doc, TOC_PAGE_MAP, USE_NATIVE_TOC
