...
- PDF template parsing requires Poppler command-line tools on `PATH`: `pdfinfo` and `pdftotext`. Missing tools must become `PDF_TEMPLATE_DEPENDENCY_MISSING` with `resume_scope=environment`; unreadable/corrupt PDFs must become `PDF_TEMPLATE_READ_FAILED` with re-export/openable-PDF guidance; scanned/textless PDFs should become `PDF_TEMPLATE_UNSUPPORTED`, not silent defaults. Sparse instruction PDFs should continue only with `PDF_TEMPLATE_INSTRUCTION_INCOMPLETE` / `PDF_TEMPLATE_LIMITED_CONFIDENCE` warnings that name the missing rule families and the next review step. Landscape PDF templates should continue with `PDF_TEMPLATE_LANDSCAPE_PAGE` warning guidance for Word/WPS page-orientation review.
...
- PDF templates are best-effort format sources: instruction-style PDFs provide text rules, sparse instruction PDFs must surface `PDF_TEMPLATE_INSTRUCTION_INCOMPLETE` warning guidance for missing rule families, landscape PDFs must surface `PDF_TEMPLATE_LANDSCAPE_PAGE` warning guidance for page-orientation review, visual sample PDFs provide estimated geometry/styles, missing Poppler tools must surface `PDF_TEMPLATE_DEPENDENCY_MISSING` with environment repair guidance, corrupt/unreadable PDFs must surface `PDF_TEMPLATE_READ_FAILED` with re-export/openable-PDF guidance, and scanned/textless PDFs must surface `PDF_TEMPLATE_UNSUPPORTED` before generation so users do not receive a misleading DOCX built from default formatting.
