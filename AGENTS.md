...
- It preserves visual QA options such as explicit golden baseline path, update-golden mode, and WPS requirement across repair rounds.
...
- Tracked-change DOCX content must be parsed as deterministic Word final-view content where possible: `w:ins` / `w:moveTo` is visible body/table/table-row/table-cell/heading/textbox-recovery content, while `w:del` / `w:moveFrom` and comment bodies must not enter the final paper. Source audit should still surface `TRACKED_CHANGES_PRESENT` / `COMMENTS_PRESENT` so ordinary users know to accept/reject revisions and confirm comments before delivery.
...
- DOCX table-cell block, inline, and nested inline content controls must keep visible `w:sdtContent` text inside the source table cell, including visible `w:fldSimple` results, `w:customXml` / `w:smartTag` wrapped visible values, and hyperlink-wrapped images/formulas/notes, and must not be duplicated by the body-level content-control fallback. Body-level content controls that partially overlap or exactly match table-cell text must still be preserved as body content.
- DOCX body-level `w:sdtContent` wrappers around paragraphs or tables must be parsed in source order; wrapped paragraphs count as recovered content-control body paragraphs, wrapped tables remain structured tables, and table-cell text must not leak as loose body text.
- DOCX body paragraph inline containers (`w:sdt`, `w:fldSimple`, hyperlink, `w:customXml`, `w:smartTag`) must be recursively consumed so images, OMML/LaTeX formulas, and footnote/endnote anchors inside content controls keep source order and are not re-appended by boxed fallback text recovery.
- DOCX body-level `w:customXml` / `w:smartTag` wrappers around paragraphs or tables must be transparent to body parsing; wrapped content stays in source order and later direct paragraphs must not be displaced or lost.
...
- `内容提取.md` should display structured images, including `role="image"` table-cell/body images, tables, and formulas by role; table summaries should mention table-cell image counts when present; avoid duplicate image listings; and never label non-formula structured content as `[公式]` or opaque `[结构化内容]`.
