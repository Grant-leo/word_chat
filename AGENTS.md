...
- For PDF templates, check `template_profile.md` and `格式提取.md` for PDF type, confidence, warnings, and possible `PDF_TEMPLATE_UNSUPPORTED`; scanned/textless PDF templates should fail closed after template profiling and before content extraction or `build_generated.py` creation, with `qa_report`, `qa_repair_plan`, `qa_fix_prompt`, and `agent_summary` telling users to provide a DOCX template, text-instruction PDF, or OCRed PDF before rerunning.
...
- PDF templates are best-effort format sources: instruction-style PDFs provide text rules, visual sample PDFs provide estimated geometry/styles, and scanned/textless PDFs must surface `PDF_TEMPLATE_UNSUPPORTED` before generation so users do not receive a misleading DOCX built from default formatting.
