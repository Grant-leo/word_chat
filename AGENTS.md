...
- Markdown H1 document titles may start with a UTF-8 BOM on Windows or use Setext `Title` + `===`; strip the BOM before heading detection, store CJK titles as `title_cn` and non-CJK titles as `title_en`, and avoid false `TITLE_MISSING` warnings for valid H1 titles. Treat Setext `---` conservatively because it conflicts with delimiters and horizontal rules.
