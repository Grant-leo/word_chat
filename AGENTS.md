...
- PDF template parsing requires Poppler command-line tools on `PATH`: `pdfinfo` and `pdftotext`. Missing tools must become `PDF_TEMPLATE_DEPENDENCY_MISSING` with `resume_scope=environment`; scanned/textless PDFs should become `PDF_TEMPLATE_UNSUPPORTED`, not silent defaults.
...
- For PDF templates, check `template_profile.md` and `格式提取.md` for PDF type, confidence, warnings, and possible `PDF_TEMPLATE_DEPENDENCY_MISSING` / `PDF_TEMPLATE_UNSUPPORTED`; missing Poppler tools should fail closed after template profiling with `resume_scope=environment` and a Poppler repair/rerun step, while scanned/textless PDF templates should fail closed before content extraction or `build_generated.py` creation with DOCX/text-instruction-PDF/OCR guidance.
...
- PDF templates are best-effort format sources: instruction-style PDFs provide text rules, visual sample PDFs provide estimated geometry/styles, missing Poppler tools must surface `PDF_TEMPLATE_DEPENDENCY_MISSING` with environment repair guidance, and scanned/textless PDFs must surface `PDF_TEMPLATE_UNSUPPORTED` before generation so users do not receive a misleading DOCX built from default formatting.
